...
// dans le modele "deleteproduct"
   ...
   if(isset($_GET['val'])){
    ...
    // dans le modèle selectcomm 
    //$unControleur->selectcomm($sesid);
    ...
    if(!$ordid)
     ...
     { 
       $sql="INSERT INTO orders (client_id,product_id,quantity,order_id) VALUES('$sesid','$key','$value','$ordid')";
        ...
        $sqlstock= "UPDATE products INNER JOIN orders SET quantite=quantite-$value WHERE id=$key ";
        ...
        //pour qu'il puisse commander il faut que la différence entre le stock du produit et la quantitée commandée soit supèrieure ou égale à 0
    }
